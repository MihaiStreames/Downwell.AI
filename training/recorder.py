...
def main():
    ...
    while True:
        ...
        memory_features = np.array([hp, player.get_value('gems') or 0, player.get_value('combo') or 0, xpos, ypos,
                                    player.get_value('ammo') or 0], dtype=np.float32)
        ...
        if len(gameplay_data) % 1000 == 0:
            ...
            filepath = os.path.join(DATA_DIR, f"session_{session_id}_chunk_{chunk_num}.pkl")
